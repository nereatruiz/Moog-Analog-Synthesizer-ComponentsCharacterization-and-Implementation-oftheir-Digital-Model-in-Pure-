...
# Sampling config
PRESETS_TO_SAMPLE = [1, 2]  # Will be selected on the synth using MIDI Program Change messages
NOTE_SECONDS = 4  # Note sustain (time between note on and note off messages)
...
VELOCITIES = [16, 53, 90, 127]  # MIDI velocities to sample for each note
...
print('Will use audio device "{0}" (change that by setting \'AUDIO_DEVICE_INDEX\' in sampler.py)'.format(p.get_device_info_by_index(AUDIO_DEVICE_INDEX)['name']))
...
def sample_note(midi_note, midi_vel, output_filename, stream):
    frames = []
...
textual_description_template = """Single note sampled from a Moog Slim Phatty analogue synthesizer. <a href="https://www.moogmusic.com/products/slim-phatty/">Slim Phatty</a> is a monophonic analogue synthesizer designed by Moog Music.
...
def sample_preset(preset_number):
    ...
    for count, (midi_note, midi_vel) in enumerate(notes):
        ...
        output_filename = '%s/Moog Slim Phatty preset %.3i - %.3i (%s) - %.3i.wav' % (RECORDING_FOLDER, preset_number, midi_note, note_name, midi_vel)
        csv_rows.append([  # Prepare CSV information for Freesound bulk description
            output_filename.split('/')[1],  # Audio filename
            'Moog Slim Phatty preset #%i - %s (%i) - vel %i' % (preset_number, note_name, midi_note, midi_vel), # Sound name
            ' '.join(['multisample single-note analogue synthesizer moog Moog-Slim-Phatty %s midi-note-%i midi-velocity-%i' % (note_name.replace('#', 'Sharp'), midi_note, midi_vel)]),  # Tags
            '',  # Geotag
