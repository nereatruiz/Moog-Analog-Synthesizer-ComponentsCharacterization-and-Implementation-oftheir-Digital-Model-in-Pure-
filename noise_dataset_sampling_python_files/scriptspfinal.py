...
NOTE_SECONDS = 20  # Note sustain (time between note on and note off messages)
TAIL_SECONDS = 20  # Time recording after note off message
...
tup = [23, 24, 25, 26, 27, 28, 29, 30, 31, 19, 21]

with open('parameters.csv') as f:
    reader = csv.reader(f)
    x = [[round(127 * float(i)) for i in l] for l in reader]

tup = [23, 24, 25, 26, 27, 28, 29, 30, 31, 19, 21]

z = [list(zip(tup, y)) for y in x]

dict_of_z = dict(enumerate(z, start=3))
PARAMETERS_CSV = list(enumerate(z, start=3))
